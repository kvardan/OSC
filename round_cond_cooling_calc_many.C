...
*/
  ...
  c_pow1->SaveAs(Form("gif/all_cooling_plots/autosave_power_set_%d_%s.gif", sett[i_set], name[i_set].c_str()));
  ...
  c_pres1->SaveAs(Form("gif/all_cooling_plots/autosave_pressure_set_%d_%s.gif", sett[i_set], name[i_set].c_str()));
  ...
  c_cden1->SaveAs(Form("gif/all_cooling_plots/autosave_cur_density_set_%d_%s.gif", sett[i_set], name[i_set].c_str()));
  ...
  c_cur->SaveAs(Form("gif/all_cooling_plots/autosave_cur_single_set_%d_%s.gif", sett[i_set], name[i_set].c_str()));
  ...
  c_lcond->SaveAs(Form("gif/all_cooling_plots/autosave_lcond_set_%d_%s.gif", sett[i_set], name[i_set].c_str()));
  ...
  c_steel->SaveAs(Form("gif/all_cooling_plots/autosave_steel_weight_set_%d_%s.gif", sett[i_set], name[i_set].c_str()));
  ...
  c_volts->SaveAs(Form("gif/all_cooling_plots/autosave_voltage_set_%d_%s.gif", sett[i_set], name[i_set].c_str()));
  ...
  c_resist->SaveAs(Form("gif/all_cooling_plots/autosave_resistance_set_%d_%s.gif", sett[i_set], name[i_set].c_str()));
  ...
  c_NI->SetGridy();
  hh_NI->Draw("colztext");
  c_NI->SaveAs(Form("gif/all_cooling_plots/autosave_NI_set_%d_%s.gif", sett[i_set], name[i_set].c_str()));
...
void cooling_calc(double nx1, double ny1, double field1)
{
  ...
  double reynolds_n_per_wire[100]={0};
  double power_per_wire[100]={0};
...
//    cout<<reynolds<<endl;
    ...
    {
      delp = delpturb;
      cout<<"1. turbulent delp="<<delp<<"     reynolds="<<reynolds<<endl;
    }
    else if (reynolds <= 2500)
    {
      delp = delplam;
      cout<<"2. laminar delp="<<delp<<"     reynolds="<<reynolds<<endl;
    }
    else
    {
      delp = delpturb*(reynolds-2500.)+delplam*(4000.-reynolds);
      delp = delp/(4000.-2500.);
      cout<<"3. between delp="<<delp<<"     reynolds="<<reynolds<<endl;
    }
...
//    cout<<endl<<endl;
    ...
    reynolds_n_per_wire[i_layer]=reynolds;
    power_per_wire[i_layer]=powcoil/1000.;
  ...
  if(plot_individual)
  {
    for (int i_tmpp=0; i_tmpp<n_l; i_tmpp++)
    {
      cout << "power = " << power_per_wire[i_tmpp]<<",         lcond = " << lcond_per_wire[i_tmpp]<<",           pressure = "<<pres[i_tmpp]*14.5038<<",           reynolds = "<<reynolds_n_per_wire[i_tmpp]<<endl;
    }

    TGraph * gr_pow = new TGraph(n_l, n_layer, power_per_wire);
    ...
    gr_pow->SetTitle("Power vs layer number (1st is at beam pipe)");
    gr_pow->GetXaxis()->SetTitle("Layer number");
    gr_pow->GetYaxis()->SetTitle("Power per layer [kW]");
    gr_pow->SetMarkerStyle(22);
